---
title: "DATA606 Lab3 - The normal distribution"
author: "JM"
date: "May 24, 2017"
output:
  html_document:
...
#### The distribution for men's height looks more normal like than the female's. The latter seems to have a larger concentration within one std deviation than the former.
...
#### Yes, it does appear that the data follows a nearly normal distribution.
...
```{r qqsimnorm, eval=TRUE}
qqnorm(sim_norm)
qqline(sim_norm)
```
#### The plot looks very similar to the original plot, though there seems to be more elements which are on the line or with almost deviation.
...
#### Using the above observations we see that there are too many outliers and deviations from the line, hence it seems the female weight is not nearly normal.
...
#### How many females are below 160? and How many females are below 60 units?
```{r qs6}
expH <- pnorm(q = 160, mean = fhgtmean, sd = fhgtsd)
print(paste0("Expected prob from Normal for Female Height: ", expH))
actH <- sum(fdims$hgt < 160) / length(fdims$hgt)
print(paste0("Actual value for Female Height: ", actH))
expW <- pnorm(q = 60, mean = fwgtmean, sd = fwgtsd)
print(paste0("Expected prob from Normal for Female Weight: ", expW))
actW <- sum(fdims$wgt < 60) / length(fdims$wgt)
print(paste0("Actual value for Female Weight: ", actW))
```
